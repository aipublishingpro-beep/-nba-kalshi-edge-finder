...
def place_kalshi_order(ticker, side, price_cents, count, api_key, private_key_pem):
    ...
    try:
        ...
        order_data = {"ticker": ticker, "action": "buy", "side": "no", "count": count, "type": "limit", "no_price": price_cents, "client_order_id": str(uuid.uuid4())}
...
def play_alert_sound(alert_type="edge"):
    sounds = {"edge": {"freq": 800, "duration": 5}, "watchlist": {"freq": 600, "duration": 5}, "mispriced": {"freq": 1000, "duration": 5}, "q1_ended": {"freq": 500, "duration": 5}}
    sound = sounds.get(alert_type, sounds["edge"])
    js_code = f"""<script>(function(){{const a=new(window.AudioContext||window.webkitAudioContext)();const o=a.createOscillator();const g=a.createGain();o.connect(g);g.connect(a.destination);o.frequency.value={sound['freq']};o.type='sine';g.gain.setValueAtTime(0.3,a.currentTime);g.gain.exponentialRampToValueAtTime(0.01,a.currentTime+{sound['duration']});o.start(a.currentTime);o.stop(a.currentTime+{sound['duration']});}})();</script>"""
...
def fetch_extreme_totals(min_threshold=245):
    ...
    try:
        ...
        if resp.status_code != 200:
            return [], f"API Error: {resp.status_code}", today
        data = resp.json()
        markets = []
...
def render_bid_recommendation(no_ask, live_data, ticker, watchlist_team=None, market_ticker=None):
    ...
    bid, label, explanation = calculate_recommended_bid(no_ask, game_state, q1_total, spiked, q1_lock_imminent)
    ...
    if spiked:
        ...
        with st.container():
            st.markdown(f"""
            <div style="background-color: #FFD700; padding: 15px; border-radius: 8px; border: 2px solid #FFA500; margin-bottom: 10px;">
            ...
            if show_trading:
                ...
                with col2:
                    if st.button(f"🚀 PLACE BID {bid}¢ × {num_contracts}", key=f"place_{ticker}", type="primary", use_container_width=True):
                        success, msg = place_kalshi_order(market_ticker or ticker, "no", bid, num_contracts, st.session_state.kalshi_api_key, st.session_state.kalshi_private_key)
                        if success:
                            st.success(f"✅ {msg}")
            ...
            if show_trading:
                ...
                with col2:
                    if st.button(f"🚀 LIFT ASK {int(no_ask)}¢ × {num_contracts}", key=f"lift_{ticker}", type="primary", use_container_width=True):
                        success, msg = place_kalshi_order(market_ticker or ticker, "no", int(no_ask), num_contracts, st.session_state.kalshi_api_key, st.session_state.kalshi_private_key)
                        if success:
                            st.success(f"✅ {msg}")
                            play_alert_sound("edge")
                        else:
...
with st.sidebar:
    ...
    if active_spikes > 0:
        st.error(f"⚠️ {active_spikes} ACTIVE SPIKE(S)")
    ...
    if watchlist_green:
        ...
        for m in watchlist_green:
            ...
            if spiked or is_spiked(m["ticker"]):
                st.error(f"🛑 **PRICE SPIKE +{int(delta)}¢** — {m['away_team']} @ {m['home_team']}")
...
st.divider()
st.caption("v5.3 | One-Click Trading | Secrets Support")
